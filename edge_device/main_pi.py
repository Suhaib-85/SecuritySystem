...
try:
    ...
    print(f"[BUZZER] init failed ({e}) — running without audible alert.")
    buzzer = None
...
print("\n[CORE] Capture loop engaged. (Ctrl+C to stop)")
...
try:
    while True:
        ...
        try:
            ...
            print(f"[CAMERA] Capture exception ({e}) — retrying...")
            time.sleep(0.5)
            continue
        if frame is None:
            time.sleep(0.1)
        ...
        if is_system_armed:
            ...
            if not intrusion_active and ai_window_counter > 0:
                ...
                if (not ignore_pir_extension
                    ...
                    print(
                        f"[RECHECK] [{current_time:.1f}s] Sustained motion — re-verifying human "
                    ...
                    if human_present:
                        ...
                        print(f"[RECHECK] Human still present — intrusion continues.")
                    else:
                        ignore_pir_extension = True  # stop PIR from extending the session
                        print(
                            f"[RECHECK] No human in {SUSTAINED_RECHECK_FRAMES} frames — ignoring further PIR; "
    ...
    print("\n[SHUTDOWN] Ctrl+C — releasing hardware...")
finally:
    try:
        if video_writer:
            video_writer.release()
